  ...
  return (
    <div className="relative flex flex-col w-full">
      ...
      <div className="border-l border-r border-neutral-700 flex flex-col gap-4 px-6 py-6">
        <h3 className="text-base font-medium text-white">Skills & Expertise</h3>
        <div className="flex flex-wrap gap-2">
          {skills.map((skill, index) => (
            <span
              key={index}
              className="px-3 py-1.5 bg-neutral-800 text-sm text-white rounded-lg border border-neutral-700"
            >
              {skill}
            </span>
          ))}
